...
do_lipo () {
    ...
    do
        LIPO_FLAGS="$LIPO_FLAGS $UNI_BUILD_ROOT/build/fdk-aac-$ARCH/output/lib/$LIB_FILE"
    done
...
do_lipo_all () {
    ...
    cp -R $UNI_BUILD_ROOT/build/fdk-aac-armv7/output/include $UNI_BUILD_ROOT/build/universal/
}

#----------
if [ "$FF_TARGET" = "armv7" -o "$FF_TARGET" = "armv7s" -o "$FF_TARGET" = "arm64" ]; then
    echo_archs
    sh tools/do-compile-fdk-aac.sh $FF_TARGET
    ...
    for ARCH in $FF_ALL_ARCHS
    do
        sh tools/do-compile-fdk-aac.sh $ARCH
    done
    ...
    echo "Usage:"
    echo "  compile-fdk-aac.sh armv7|arm64|i386|x86_64"
    echo "  compile-fdk-aac.sh armv7s (obselete)"
    echo "  compile-fdk-aac.sh lipo"
    echo "  compile-fdk-aac.sh all"
