...
- Because the company project requirements, must put fdk-aac video compiled files are added to the ffmepg repository
- 因公司项目需求，必须把fdk-aac视频编译文件添加到ffmepg库里面
- Oneself fumble with git lookup can made a compilation of fdk-aac library
- 自己摸索加git查找做了一个能编译fdk-aac库
- To compile fdk-aac library into ffmpeg, and need to compile fdk-aac library
- 要把fdk-aac库编译进入ffmpeg里面，需要先编译fdk-aac库
- Will need fdk-aac before compilation of ffmpeg, relevant agreements in ffmpeg configuration file to open
- 在编译ffmpeg之前需要将fdk-aac，相应协议在ffmpeg配置文件里面打开（配置已被打开 --enable-decoder=libfdk-aac）
